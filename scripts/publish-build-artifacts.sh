...
if [ -z "$NGX_SIMPLE_HTTP_BUILDS_TOKEN" ]; then
  echo -e "\x1b[31m\x1b[1mError: No access token for GitHub could be found." \
    "Please set the environment variable 'NGX_SIMPLE_HTTP_BUILDS_TOKEN'.\x1b[0m" >&2
  exit 1
fi

echo -e "\x1b[34mDEBUG: Current directory: $(pwd)\x1b[0m"
# Material packages that need to be published.
PACKAGES=(ngx-simple-http)
REPOSITORIES=(ngx-simple-http-builds)
PACKAGE_LOCATIONS=(ngx-simple-http)
...
publishPackage() {
  ...
  buildVersion=$(node -pe "require('$TRAVIS_BUILD_DIR/projects/ngx-simple-http/package.json').version")
  ...
  echo "https://${NGX_SIMPLE_HTTP_BUILDS_TOKEN}:@github.com" >.git/credentials
